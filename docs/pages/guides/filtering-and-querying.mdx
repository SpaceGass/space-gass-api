...
```python title="Python"
filtered = await client.job.structure.nodes.query(
...
```python title="Python"
result = await client.job.query.analysis.static.node_displacements.query(
    cases="1,3", nodes="10-12")
```
...
```python title="Python"
result = await client.job.query.analysis.static.member_intermediate_forces.query(
    members="1")

for m in result.results:
    print(f"Case {m.case}, Member {m.member}:")
...
for b in result.results:
    print(f"Case {b.case}, Mode {b.mode}: "
...
for f in result.results:
    print(f"Case {f.case}, Mode {f.mode}: "
...
```python title="Python"
page1 = await client.job.query.analysis.static.node_displacements.query(
    offset=0, limit=100)

page2 = await client.job.query.analysis.static.node_displacements.query(
