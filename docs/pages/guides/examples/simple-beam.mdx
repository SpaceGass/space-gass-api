...
`MemberIntermediateForce` returns `Mz` sampled at several stations
along each member as a parallel array; filter by ULS case + the member
Id and take the maximum of `|Mz|`.
...
combination. `MemberIntermediateDisplacement.TyGlobal` is the global-Y
translation at each station; filter by SLS case + the member Id and
take the maximum of `|TyGlobal|`.
