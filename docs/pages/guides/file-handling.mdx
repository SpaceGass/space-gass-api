...
var multipart = new MultipartBody { RequestAdapter = client.RequestAdapter };
using var stream = File.OpenRead(@"C:\Templates\my-template.sgbase");
multipart.AddOrReplacePart("template", "application/octet-stream", stream);

await client.Job.NewFromTemplate.PostAsync(multipart);
```
...
multipart.request_adapter = client.request_adapter
with open(r"C:\Templates\my-template.sgbase", "rb") as f:
    multipart.add_or_replace_part("template", "application/octet-stream", f)
    await client.job.new_from_template.post(multipart)
```
