...
1. **Probe** — try `Service.Status` to see if the service is already
   running. If yes, reuse it (don't kill someone else's instance).
2. **Start** — if not running, launch `SpaceGassApi.exe` as a child
   process.
3. **Wait** — poll `Service.Status` until it responds, or fail with a
...
`Service.Status` to confirm, then shuts the service down.
