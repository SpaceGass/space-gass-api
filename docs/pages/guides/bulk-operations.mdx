...
| **Failure response** | Throws on the first failing item | Returns a `*BatchResult` with `Succeeded` and `Errors` lists |
...
Bulk endpoints accept a `List<XCreate>` and return an `XBatchResult`
with the created entities under `Succeeded` and any failures under
`Errors`. `BatchError.Index` is the position in the input list, so you
