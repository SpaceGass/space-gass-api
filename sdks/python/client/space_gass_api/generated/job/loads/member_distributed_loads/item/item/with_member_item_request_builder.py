...
class WithMemberItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /job/loads/member-distributed-loads/{caseId}/{memberId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/job/loads/member-distributed-loads/{caseId}/{memberId}", path_parameters)
    
    def by_sub_load_id(self,sub_load_id: int) -> WithSubLoadItemRequestBuilder:
        """
        Gets an item from the space_gass_api.generated.job.loads.memberDistributedLoads.item.item.item collection
