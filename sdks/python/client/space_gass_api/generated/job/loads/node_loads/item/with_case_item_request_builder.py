...
class WithCaseItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /job/loads/node-loads/{caseId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/job/loads/node-loads/{caseId}", path_parameters)
    
    def by_node_id(self,node_id: int) -> WithNodeItemRequestBuilder:
        """
        Gets an item from the space_gass_api.generated.job.loads.nodeLoads.item.item collection
