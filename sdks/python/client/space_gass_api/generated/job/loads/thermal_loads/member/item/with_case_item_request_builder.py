...
class WithCaseItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /job/loads/thermal-loads/member/{caseId}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/job/loads/thermal-loads/member/{caseId}", path_parameters)
    
    def by_member_id(self,member_id: int) -> WithMemberItemRequestBuilder:
        """
        Gets an item from the space_gass_api.generated.job.loads.thermalLoads.member.item.item collection
