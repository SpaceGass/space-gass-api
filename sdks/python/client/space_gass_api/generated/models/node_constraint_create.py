...
class NodeConstraintCreate(Parsable):
    ...
    # A 6-character string defining which degrees of freedom are constrained.Each character position maps to a DOF: UX, UY, UZ, RX, RY, RZ (left to right).'F' = Fixed (tied to master), 'R' = Released (free).Example: "FFFRRR" constrains translations only; "FFFFFF" constrains all 6 DOFs.
    ...
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> NodeConstraintCreate:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: NodeConstraintCreate
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return NodeConstraintCreate()
