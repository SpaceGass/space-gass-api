...
@dataclass
class SteelMemberCreate(Parsable):
    """
    DTO for creating a new steel member design data entity (design group).The Id is the design group number (auto-assigned if omitted).`members` is required; all other fields are optional and take the row defaults when omitted.
    """
    ...
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> SteelMemberCreate:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: SteelMemberCreate
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return SteelMemberCreate()
