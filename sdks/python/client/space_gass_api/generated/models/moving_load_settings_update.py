...
@dataclass
class MovingLoadSettingsUpdate(Parsable):
    """
    ...
    """
    # Apply member loads to the closest member only. Omit to keep current.
    apply_to_closest_member: Optional[bool] = None
    # Check vertical proximity when applying loads to members. Omit to keep current.
    check_vertical_proximity: Optional[bool] = None
    # Ignore loads that transfer load to just one member. Omit to keep current.
    ignore_loads_on_one_member: Optional[bool] = None
    # Ignore loads that are outside the loading area. Omit to keep current.
    ignore_outside_loaded_area: Optional[bool] = None
    # Keep loads entirely within the ends of the travel path (crane-beam behaviour). Omit to keep current.
    keep_loads_within_travel_path: Optional[bool] = None
    # Retain generated loads for deselected scenarios. Omit to keep current.
    retain_loads: Optional[bool] = None
    # Maximum vertical distance for a load to be applied. Omit to keep current.
    vertical_proximity: Optional[float] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> MovingLoadSettingsUpdate:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: MovingLoadSettingsUpdate
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return MovingLoadSettingsUpdate()
