...
@dataclass
class FilterPlateThicknesses(Parsable):
    """
    ...
    # Plate thicknesses to match against. Values are in the model's Length unit.Maximum 10 values; empty array means no thickness criterion.
    plate_thicknesses: Optional[list[float]] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> FilterPlateThicknesses:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: FilterPlateThicknesses
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return FilterPlateThicknesses()
