...
@dataclass
class FilterAxisRange(Parsable):
    """
    ...
    """
    # Whether the upper bound is inclusive (`true`: `value <= limitHigh`)or strict (`false`: `value < limitHigh`).
    high_inclusive: Optional[bool] = None
    # Whether this sub-filter participates in the filter.
    is_active: Optional[bool] = None
    # Upper bound of the range. Unit: Length (see `GET /job/units`).
    limit_high: Optional[float] = None
    # Lower bound of the range. Unit: Length (see `GET /job/units`).
    limit_low: Optional[float] = None
    # Whether the lower bound is inclusive (`true`: `value >= limitLow`)or strict (`false`: `value > limitLow`).
    ...
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> FilterAxisRange:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: FilterAxisRange
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return FilterAxisRange()
