...
@dataclass
class FilterPlateCuts(Parsable):
    """
    ...
    # Plate-cut Ids in SG list format. Empty string means no Id criterion.
    plate_cuts: Optional[str] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> FilterPlateCuts:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: FilterPlateCuts
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return FilterPlateCuts()
