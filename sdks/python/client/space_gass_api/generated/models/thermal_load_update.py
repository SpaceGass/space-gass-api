...
@dataclass
class ThermalLoadUpdate(Parsable):
    """
    DTO for updating an existing thermal load.Only fields included in the request are updated; omit a field to keep its current value.
    """
    # The load case number.
    case: Optional[int] = None
    # The element Id (member number or plate number).
    ...
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> ThermalLoadUpdate:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: ThermalLoadUpdate
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return ThermalLoadUpdate()
