...
class ThermalLoadKey(Parsable):
    ...
    # The element key (member number or plate number, depending on ElementType).
    key: Optional[int] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> ThermalLoadKey:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: ThermalLoadKey
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return ThermalLoadKey()
