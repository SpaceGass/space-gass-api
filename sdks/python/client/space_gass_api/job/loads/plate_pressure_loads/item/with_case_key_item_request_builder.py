...
class WithCaseKeyItemRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /job/loads/plate-pressure-loads/{caseKey}
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/job/loads/plate-pressure-loads/{caseKey}", path_parameters)
    
    def by_plate_key(self,plate_key: int) -> WithPlateKeyItemRequestBuilder:
        """
        Gets an item from the space_gass_api.job.loads.platePressureLoads.item.item collection
