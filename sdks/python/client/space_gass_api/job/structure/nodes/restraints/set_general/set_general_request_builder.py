...
class SetGeneralRequestBuilder(BaseRequestBuilder):
    """
    Builds and executes requests for operations under /job/structure/nodes/restraints/set-general
    """
    def __init__(self,request_adapter: RequestAdapter, path_parameters: Union[str, dict[str, Any]]) -> None:
        ...
        Returns: None
        """
        super().__init__(request_adapter, "{+baseurl}/job/structure/nodes/restraints/set-general", path_parameters)
    ...
    async def post(self,body: SetGeneralRestraintRequest, request_configuration: Optional[RequestConfiguration[QueryParameters]] = None) -> Optional[NodeRestraint]:
        """
        Promotes a single node as the general restraint and atomically demotes every other row.Pass `node = N` to set node N as the general restraint;pass `node = null` to clear the flag from every node so no general restraint is set.            At most one node can be the general restraint at any time. The target node must alreadyhave a restraint row — use `POST /{id}/restraint` to create one first.            To observe which node (if any) is currently the general restraint, read`generalRestraint` on the node restraint DTO via `GET /restraints` or`GET /{id}/restraint`.
        param body: Request body for `POST job/structure/nodes/restraints/set-general`.Provide a node Id to promote that node as the general restraint(demoting every other row); pass `null` to clear the flag from every node.
